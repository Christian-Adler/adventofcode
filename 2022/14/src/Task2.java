...
public class Task2 {
    ...
    public void afterParse() {
        ...
        while (sandMoving) {
            xMin = Math.min(xMin, fallingSand.x);
            xMax = Math.max(xMax, fallingSand.x);
            ...
            if (checkPos.y == yMax) {
                map.put(checkPos, ROCK);
            } else if (map.get(checkPos) == null) {
                fallingSand.y++;
                continue;
            } else if (map.get(new Pos(fallingSand.x - 1, fallingSand.y + 1)) == null) {
                fallingSand.x--;
                fallingSand.y++;
                continue;
            } else if (map.get(new Pos(fallingSand.x + 1, fallingSand.y + 1)) == null) {

        ...
        while (rockCount < maxRockCount) {
            ...
            if (foundRepeat) {
                ...
                if (stored.equals(toString())) {
                    ...
                    // Unbedingt neue Keys in der Map erstellen - nicht die bestehenden veraendern!
                    ArrayList<Pos> keys = new ArrayList<>(map.keySet());
                    for (Pos pos : keys) {
                        Pos p = new Pos(pos.x, pos.y + aufAlleYDrauf);
                        map.put(p, true);
                        last100.add(p);
                        towerHeight = Math.max(towerHeight, p.y + 1);
                    }

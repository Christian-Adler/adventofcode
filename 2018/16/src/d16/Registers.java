...
public class Registers {
    protected ArrayList<Integer> regis = new ArrayList<>(Arrays.asList(0, 0, 0, 0));

    public Registers() {
    }

    public Registers(String input) {
        regis = new ArrayList<>(Arrays.stream(Util.cleanFrom(input, "Before:", "After:", "[", "]", ",").trim().split("\\s+")).mapToInt(Integer::parseInt).boxed().toList());
        if (regis.size() != 4) throw new IllegalStateException("d16.Registers size != 4");
    }

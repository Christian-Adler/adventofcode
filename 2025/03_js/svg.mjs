...
import {hslToRgbStr} from "./colorutils.mjs";
...
export class Svg {
  constructor(zoomFactor) {
    ...
    this.posMap = new Map();
    this.zoomFactor = (typeof zoomFactor === 'number' && zoomFactor > 0) ? zoomFactor : 1;
  }

  add(pos, color = undefined) {
    const x = pos.x;
    const y = pos.y;
    ...
    const p = new Vector(x, y, color ? color : pos.color);
